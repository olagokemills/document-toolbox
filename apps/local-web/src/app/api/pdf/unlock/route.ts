import { unlockPdf } from '@private-pdf/pdf-core'
...
export async function POST(req: Request) {
  try {
    ...
    if (!password) {
      return Response.json({ error: 'Please enter the current password.' }, { status: 400 })
    }
    ...
    const result = await unlockPdf(inputFile, { password })
